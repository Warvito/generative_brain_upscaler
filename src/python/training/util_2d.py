    ...
    if model_type == "controlnet":
        val_transforms = transforms.Compose(
            [
                ...
                transforms.Rotate90d(
                    keys=["t1w", "flair"], k=-1, spatial_axes=(0, 1)
                ),  # Fix flipped image read
                transforms.Flipd(
                    keys=["t1w", "flair"], spatial_axis=1
                ),  # Fix flipped image read
                transforms.ScaleIntensityRanged(
                    keys=["t1w", "flair"],
                    a_min=0.0,
        ...
        train_transforms = transforms.Compose(
            [
                ...
                transforms.RandShiftIntensityd(
                    keys=["t1w", "flair"], offsets=0.05, prob=0.1
                ),
                transforms.RandAdjustContrastd(
                    keys=["t1w", "flair"], gamma=(0.97, 1.03), prob=0.1
                ),
                transforms.ThresholdIntensityd(
                    keys=["t1w", "flair"], threshold=1, above=False, cval=1.0
                ),
                transforms.ThresholdIntensityd(
                    keys=["t1w", "flair"], threshold=0, above=True, cval=0
                ),
                ApplyTokenizerd(keys=["report"]),
                transforms.RandLambdad(
                    keys=["report"],
